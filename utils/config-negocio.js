...
// CLIENTE: Robles Nails Home Studio
...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '0e63f543-ddb4-4911-aa02-e85062adf063'; // ID de Robles Nails Home Studio
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Robles Nails Home Studio';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '55816600';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'tamaranails0024@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Robles Nails Home Studio!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'roblesnailshomestudio';
};
...
console.log('✅ config-negocio.js listo para Robles Nails Home Studio');
